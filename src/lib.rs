...
static REGULAR_LANGUAGE_RANGE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?x) ^
        (?P<language> (?:
...
static LANGUAGE_RANGE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?x) ^
        (?: [[:alpha:]]{1,8} | \* )
        (?: - (?: [[:alnum:]]{1,8} | \* ))*
...
static UNIX_INVARIANT_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?ix) ^
        (?: c | posix )
        (?: \. (?: [0-9a-zA-Z-]{1,20} ))?
...
static UNIX_TAG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?ix) ^
...
fn is_owned<T: ToOwned + ?Sized>(c: &Cow<T>) -> bool {
    match c {
        Cow::Owned(_) => true,
...
fn canon_lower(o: Option<&str>) -> Cow<str> {
    match o {
        None => Cow::Borrowed(""),
        Some(s) => {
...
fn canon_script(o: Option<&str>) -> Cow<str> {
...
fn canon_upper(o: Option<&str>) -> Cow<str> {
...
impl<'a> LanguageRange<'a> {
    ...
    /// [RFC5646]: https://www.rfc-editor.org/rfc/rfc5646.txt
    /// [RFC4647]: https://www.rfc-editor.org/rfc/rfc4647.txt
    pub fn new(lt: &str) -> Result<LanguageRange> {
        if lt.is_empty() {
...
static LOCALE_ELEMENT_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r"(?ix) ^
...
impl Locale {
    ...
    pub fn new(s: &str) -> Result<Locale> {
        let mut i = s.split(',');
        let mut res = Locale::from(LanguageRange::new(i.next().unwrap())?); // NOTE: split "" is (""), not ()
        for t in i {
...
static USER_LOCALE: Lazy<Locale> = Lazy::new(|| system_locale());
static GLOBAL_LOCALE: Lazy<Mutex<Locale>> = Lazy::new(|| Mutex::new(Locale::user_default()));
...
impl ::std::fmt::Display for Error {
    fn fmt(&self, out: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        match self {
            Error::NotWellFormed => write!(out, "Language tag is not well-formed."),
        }
    }
}
