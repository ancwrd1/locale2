use crate::win32::consts::{
    LOCALE_IFIRSTDAYOFWEEK, LOCALE_IMEASURE, LOCALE_INEGCURR, LOCALE_SDECIMAL, LOCALE_SLIST,
    LOCALE_SMONDECIMALSEP, LOCALE_SMONTHOUSANDSEP, LOCALE_SNATIVEDIGITS, LOCALE_SSHORTDATE,
    LOCALE_STHOUSAND, LOCALE_STIMEFORMAT,
};
use std::fmt::Write;
use std::os::raw::c_ulong;
use windows_sys::Win32::Globalization::{
    GetLocaleInfoEx, GetSystemDefaultLocaleName, GetUserDefaultLocaleName,
    GetUserPreferredUILanguages,
};

use super::{LanguageRange, Locale};

fn if_locale_info_differs<F: FnOnce(&str)>(lc_type: c_ulong, func: F) {
    #[allow(non_snake_case)]
    // would be const if it wasn't for the fact const functions are still unstable
    let LOCALE_NAME_USER_DEFAULT: *mut u16 = ::std::ptr::null_mut();
    const LOCALE_NOUSEROVERRIDE: c_ulong = 0x80000000;
    ...
    if len > 0 {
...

        {
            ...
            if_locale_info_differs(LOCALE_INEGCURR, |s| {
                u_ext.push_str(match s {
                    "0" | "4" | "14" | "15" => "-cf-account",
                    _ => "-cf-standard",
                });
            });
...
fn get_system_default_locale() -> super::Result<LanguageRange<'static>> {
    let mut buf = [0u16; 85];
    let len = unsafe { GetSystemDefaultLocaleName(buf.as_mut_ptr(), buf.len() as i32) };
    if len > 0 {
...
const MUI_LANGUAGE_NAME: c_ulong = 0x8; // from winnls.h
...
fn get_user_preferred_languages() -> Vec<LanguageRange<'static>> {
    ...
    let res = unsafe {
        GetUserPreferredUILanguages(MUI_LANGUAGE_NAME, &mut n_langs, buf.as_mut_ptr(), &mut len)
    };
    if res != 0 && len > 1 {
...
pub fn system_locale() -> Option<Locale> {
    let mut loc = if let Ok(lr) = get_user_default_locale() {
        Some(Locale::from(lr))
    } else {
        None
    };
    ...
    if !msg_locs.is_empty() {
        if loc.is_none() {
            ...
            loc.as_mut()
                .unwrap()
                .add_category("messages", &msg_locs.remove(0));
        }
